			...
			foreach ( $grouped_products as $grouped_product ) {
				...
				setup_postdata( $post );
					...
					<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<td class="label">
							<label for="product-<?php echo crypterio_sanitize_text_field($grouped_product->get_id()); ?>">
								<?php echo crypterio_sanitize_text_field($grouped_product->is_visible()) ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name(); ?>
							</label>
						</td>
						<?php do_action( 'woocommerce_grouped_product_list_before_price', $grouped_product ); ?>
						<td class="price">
							<?php
								echo crypterio_sanitize_text_field($grouped_product->get_price_html());
								echo crypterio_sanitize_text_field(wc_get_stock_html( $grouped_product ));
							?>
						</td>
					</tr>
					<?php
